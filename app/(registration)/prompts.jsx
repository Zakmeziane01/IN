...
const Prompts = () => {
  ...
  const handlePress = async () => {
    ...
    // First, do validation checks outside of the try block
    ...
    // Now handle the asynchronous operation inside the try block
    setIsSubmitting(true);
    try {
      const promptsString = JSON.stringify(selectedPrompts);
      await updateUserAttribute(user.userId, "prompts", promptsString);
      updateResponse("prompts", promptsString);
  ...
  const handlePromptChange = (selectedPrompt) => {
    setCurrentPrompt(selectedPrompt); // Set the current prompt
  };

  const handleAnswerChange = (text) => {
    setCurrentAnswer(text); // Set the current answer
  };
      ...
      <View className="w-full justify-center px-3 flex-1" style={{ height: 200 }}>
        ...
        <View className="relative">
          <TextInput
            ...
            multiline
            style={{ paddingRight: 50 }} // Add padding to the right to avoid overlap with the Pressable
          />
